...
def _make_nets(variables, config, net_assignments, number):
  ...
  if net_assignments is None:
    ...
    with tf.variable_scope("vars_optimizer"+str(number)):
...
class MetaOptimizer(object):
  ...
  def meta_loss(self,
    ...
    # to each optimizer.
    nets, net_keys, subsets = _make_nets(x, self._config, net_assignments, number)
    print('nets', nets)
    ...
    for k, net in nets.items():
      ...
      ************************************
      '''
      opt_var = [op for op in snt.get_variables_in_module(net)]
      '''
      ************************************
      '''

    return MetaLoss(loss, update, reset, fx_final, x_final), scale, x, constants, subsets, step, \
           loss_mt, update_mt, reset_multitask, mt_labels, mt_inputs, opt_var
  ...
  def meta_minimize(self, make_loss, len_unroll, learning_rate=0.01, **kwargs):
    ...
    info, scale, x, constants, subsets, seq_step, loss_mt, update_mt, reset_multitask, mt_labels, mt_inputs, opt_var = \
    ...
    **********************
    ...
    optimizer_SGD = tf.train.GradientDescentOptimizer(learning_rate=0.001)
    step_SGD = optimizer_SGD.minimize(info.loss)
    '''
    **********************
    '''
    ...
    *****************************
    ...
    val_opt_grad = []
    for k in opt_var:
        val_opt_grad.append(tf.placeholder(tf.float32, shape=k.shape))
    apply_grad = optimizer.apply_gradients(zip(val_opt_grad, opt_var))

    compute_grad = optimizer.compute_gradients(info.loss, opt_var)
    '''
    *****************************
    '''
    # mt
    ...
    return MetaStep(step, *info[1:]), scale, x, constants, subsets, seq_step, \
           ...
           opt_var, apply_grad, val_opt_grad, compute_grad, step_SGD
