...
def main(_):
    ...
    problem, net_config, net_assignments = util.get_config(tested_problem_name, FLAGS.path, net_name="RNNprop", stddev=FLAGS.stddev, l1_weight=0.005,mnist_classes=1)
    ...
    with ms.MonitoredSession() as sess:
        ...
        for e in xrange(FLAGS.num_epochs):
            ...
            time, cost, training_acc_cur, objective_values_cur = util.run_eval_epoch(sess, cost_op, [update],
                                             num_unrolls, step=step, unroll_len=1, var_x=var_x, all_vars=all_vars)
            total_time += time
